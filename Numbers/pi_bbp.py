"""
Calculate digits of Pi in hexademical format
...
def check_bbp(accuracy=11):  # 11 enough for digits
    ...
    for k in range(0, accuracy):
        part_sum = (4.0/(8*k + 1) - 2.0/(8*k + 4) -
                    1.0/(8*k + 5) - 1.0/(8*k + 6))/pow(16, k)
        pi += part_sum
        print k, '   ', part_sum
...
def pi_bbp(n, verbose=None):
    ...
    if verbose:
        print '\n    sum ', pi
        print '    ' + str(n) + 'th digit (the integer part of 16*sum): ',
        print hex_pi, '\n'

    return hex_pi

if __name__ == '__main__':
    print "\nCalculate digit of Pi in hexademical format \
            using Bailey-Borwein-Plouffe formula.\n"
